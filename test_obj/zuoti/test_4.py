...
visual_block.Visual_Block_Total_Block(12)
...
try:
    ...
    head_list = ['发票代码', '发票号码', '开票日期', '校验码', '机器编号', '结果', '合计(小写)', '购买方名称', '购买方纳税人识别号', '购买方地址、电话', '购买方开户行及账号',
...
try:
    ...
    rpa_6a4539_excel.get_sheet("").write("A" + str("1") + ":" + "N" + str("1"), head_list)
...
# -*- 创建None变量
try:
    visual_block.Visual_Block_Info("创建None变量", 11, 0, "正在执行")

    cre_none = None


except Exception as e:
    visual_block.Visual_Block_Info("创建None变量", 11, 2, e)
...
# -*- 遍历文件列表
try:
    visual_block.Visual_Block_Info("遍历文件列表", 12, 0, "正在执行")
            ...
            while "1" == "1":
                ...
                try:
                    ...
                    if rpa_5a6268 != cre_none:
                        ...
                        # -*- 截图文件名
                        try:
                            rpa_7a42c3_string = "截图_" + rpa_573c3d_array_any
                        ...
                        # -*- 截图的绝对路径
                        try:
                            rpa_9e704e_string = "C:\\Users\\Administrator\\Desktop\\all_test\\orientation-master-69c44478cbf4319c878dd001239880dc717ac6fb\\rpa\\入门三道题\\第三题 附件\\" + rpa_7a42c3_string
                        ...
                        try:
                            ...
                            for i in a_list:
                                d = i.encode('utf-8')
                                b_list.append("'" + d.split('\xef\xbc\x9a')[1])
                        ...
                        try:
                            ...
                            sleep(0)
                            rpa_c7ad3a.add_element("rpa_d8d68e", id="", css="", xpath=['//*[@id="cycs"]',
                                                                                       '/html/body/div[1]/div/div[2]/div/div[1]/table/tbody/tr/td/span[1]'],
                                                   frame=[0])
                            rpa_c7ad3a.switch_to_frame_by_path([0])
                            rpa_af8e9e_string = rpa_c7ad3a.get_element("rpa_d8d68e", timeout=30).text()
                            sleep(0)

                        except Exception as e:
                            log.error("[%s] %s", "获取结果", e.message)
                            raise e

                        # -*- c_list
                        try:
                            c_list = rpa_af8e9e_string

                        except Exception as e:
                            log.error("[%s] %s", "c_list", e.message)
                            raise e
                        ...
                        try:
                            ...
                            log.error("[%s] %s", "d_list", e.message)
                            raise e

                        # -*- 得到正确次数
                        try:
                            print c_list
                            c_list1 = c_list.split('\xef\xbc\x9a')
                            d_list = c_list1[1]

                        except Exception as e:
                            log.error("[%s] %s", "得到正确次数", e.message)
                            raise e
                        ...
                        try:
                            ...
                            rpa_6a4539_excel.get_sheet("").write("F" + str(row), d_list)

                        except Exception as e:
                            log.error("[%s] %s", "写入结果次数", e.message)
                            raise e
                        ...
                        try:
                            ...
                            rpa_6a4539_excel.get_sheet("").write("G" + str(row) + ":" + "N" + str(row), a_info)
                        ...
                        try:
                            ...
                            if rpa_ff4f7c_webelement != cre_none:
                                ...
                                try:
                                    ...
                                    log.error("[%s] %s", "截图文件名1", e.message)
                                    raise e

                                # -*- 截图的绝对路径1
                                try:
                                    rpa_87d4cf = "C:\\Users\\Administrator\\Desktop\\all_test\\orientation-master-69c44478cbf4319c878dd001239880dc717ac6fb\\rpa\\入门三道题\\第三题 附件\\" + rpa_a509b6

                                except Exception as e:
                                    log.error("[%s] %s", "截图的绝对路径1", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    log.error("[%s] %s", "失败页面截图", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    log.error("[%s] %s", "拼接发票代码", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    log.error("[%s] %s", "写入发票代码", e.message)
                                    raise e

                                # -*- 拼接发票号码
                                try:
                                    rpa_8403a2 = "'" + rpa_4c4ea6.get("发票号码")

                                except Exception as e:
                                    log.error("[%s] %s", "拼接发票号码", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    rpa_6a4539_excel.get_sheet("").write("B" + str(row), rpa_8403a2)

                                except Exception as e:
                                    log.error("[%s] %s", "写入发票号码", e.message)
                                    raise e

                                # -*- 拼接开票日期
                                try:
                                    rpa_26f269 = "'" + rpa_4c4ea6.get("开票日期")

                                except Exception as e:
                                    log.error("[%s] %s", "拼接开票日期", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    rpa_6a4539_excel.get_sheet("").write("C" + str(row), rpa_26f269)

                                except Exception as e:
                                    log.error("[%s] %s", "写入开票日期", e.message)
                                    raise e

                                # -*- 拼接校验码
                                try:
                                    rpa_d101da = "'" + rpa_4c4ea6.get("校验码")

                                except Exception as e:
                                    log.error("[%s] %s", "拼接校验码", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    rpa_6a4539_excel.get_sheet("").write("D" + str(row), rpa_d101da)

                                except Exception as e:
                                    log.error("[%s] %s", "写入校验码", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    rpa_6a4539_excel.get_sheet("").write("E" + str(row), '无')

                                except Exception as e:
                                    log.error("[%s] %s", "写入机器编码", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    rpa_6a4539_excel.get_sheet("").write("F" + str(row), res_str)

                                except Exception as e:
                                    log.error("[%s] %s", "写入结果", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    log.error("[%s] %s", "Excel + 1运算", e.message)
                                    raise e
                                ...
                                try:
                                    ...
                                    log.error("[%s] %s", "赋值新row", e.message)
                                    raise e
    ...
    visual_block.Visual_Block_Info("遍历文件列表", 12, 2, e)
