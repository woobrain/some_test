# -*- coding:utf-8 -*-
# import re
# # date_day = '2019年03月01日'
# # a = re.split('[年月日]',date_day)
# # print(a)
...
# -*- 获取对号
try:
    visual_block.Visual_Block_Info("获取对号", 18, 0, "正在执行")

    from time import sleep

    sleep(0)
    rpa_2ab734_webpage.add_element("rpa_a36988", id="", css=".ant-tooltip-open .anticon",
                                   xpath=["//nz-progress/div/div/span/i"], frame=[])
    rpa_2ab734_webpage.switch_to_frame_by_path([])
    rpa_d6cca0_webelement = rpa_2ab734_webpage.get_element(element_name="rpa_a36988", index=0, timeout=30)
    sleep(0)

except Exception as e:
    visual_block.Visual_Block_Info("获取对号", 18, 2, e)

    log.error("[%s] %s", "获取对号", e.message)
    raise e

# -*- 打印日志
try:
    visual_block.Visual_Block_Info("打印日志", 19, 0, "正在执行")

    from rpa import log

    log.info("[%s] %s", "打印日志", rpa_d6cca0_webelement)

except Exception as e:
    visual_block.Visual_Block_Info("打印日志", 19, 2, e)
...
# -*- 退出程序
try:
    visual_block.Visual_Block_Info("退出程序", 20, 0, "正在执行")

    exit(0)

except Exception as e:
    visual_block.Visual_Block_Info("退出程序", 20, 2, e)
...
# -*- 循环执行1
try:
    visual_block.Visual_Block_Info("循环执行1", 21, 0, "正在执行")
        ...
        try:
            ...
            if rpa_d6cca0_webelement == "True":
                ...
                try:
                    ...
                    rpa_2ab734_webpage.get_element("rpa_cd5cda", timeout=30).click(
    ...
    visual_block.Visual_Block_Info("循环执行1", 21, 2, e)
...
# -*- count
try:
    visual_block.Visual_Block_Info("count", 22, 0, "正在执行")

    count = 0

except Exception as e:
    visual_block.Visual_Block_Info("count", 22, 2, e)
...
# -*- col_list
try:
    visual_block.Visual_Block_Info("col_list", 23, 0, "正在执行")
    ...
    visual_block.Visual_Block_Info("col_list", 23, 2, e)
...
# -*- 循环执行
try:
    visual_block.Visual_Block_Info("循环执行", 24, 0, "正在执行")
        ...
        try:
            ...
            for rpa_d88d82_any in a_list:
                ...
                try:
                    ...
                    log.error("[%s] %s", "count-num + 1", e.message)
                    raise e
    ...
    visual_block.Visual_Block_Info("循环执行", 24, 2, e)
