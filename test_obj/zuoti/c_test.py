...
visual_block.Visual_Block_Total_Block(15)
...
# -*- 写入/设置 Excel 内容
try:
    visual_block.Visual_Block_Info("写入/设置 Excel 内容", 3, 0, "正在执行")

    import rpa.excel

    rpa_6a4539_excel.get_sheet("").write("A" + str("1") + ":" + "B" + str("1"), ['发票代码', '对比结果'])

except Exception as e:
    visual_block.Visual_Block_Info("写入/设置 Excel 内容", 3, 2, e)

    log.error("[%s] %s", "写入/设置 Excel 内容", str(e))
    raise e

# -*- 文件夹绝对路径
try:
    visual_block.Visual_Block_Info("文件夹绝对路径", 4, 0, "正在执行")

    a_path = "C:\\Users\\Administrator\\Desktop\\第三题 附件\\fapiao_pics\\"

except Exception as e:
    visual_block.Visual_Block_Info("文件夹绝对路径", 4, 2, e)
...
# -*- 获取文件列表
try:
    visual_block.Visual_Block_Info("获取文件列表", 5, 0, "正在执行")
    ...
    visual_block.Visual_Block_Info("获取文件列表", 5, 2, e)
...
# -*- 接收文件列表
try:
    visual_block.Visual_Block_Info("接收文件列表", 6, 0, "正在执行")

    file_list = rpa_e54673_array_string

except Exception as e:
    visual_block.Visual_Block_Info("接收文件列表", 6, 2, e)
...
# -*- 得到新的文件列表
try:
    visual_block.Visual_Block_Info("得到新的文件列表", 7, 0, "正在执行")
    ...
    visual_block.Visual_Block_Info("得到新的文件列表", 7, 2, e)
...
# -*- 数组长度
try:
    visual_block.Visual_Block_Info("数组长度", 8, 0, "正在执行")

    rpa_35b5f0_array_any = len(file_list)

except Exception as e:
    visual_block.Visual_Block_Info("数组长度", 8, 2, e)
...
# -*- 文件索引
try:
    visual_block.Visual_Block_Info("文件索引", 9, 0, "正在执行")

    count_num = 0

except Exception as e:
    visual_block.Visual_Block_Info("文件索引", 9, 2, e)
...
# -*- 创建None变量
try:
    visual_block.Visual_Block_Info("创建None变量", 10, 0, "正在执行")

    cre_none = None

except Exception as e:
    visual_block.Visual_Block_Info("创建None变量", 10, 2, e)
...
# -*- Excel行数
try:
    visual_block.Visual_Block_Info("Excel行数", 11, 0, "正在执行")

    row = 2

except Exception as e:
    visual_block.Visual_Block_Info("Excel行数", 11, 2, e)
                        ...
                        try:
                            ...
                            if "卷票" in rpa_94f34b_string:
                                ...
                                try:
                                    ...
                                    log.error("[%s] %s", "创建变量3", str(e))
                                    raise e

                                # -*- 自定义脚本2
                                try:
                                    print(rpa_08dc67_string)
                                    bjp_list = [
                                        "'" + rpa_4c4ea6.get("发票代码"),
                                        "'" + rpa_4c4ea6.get("发票号码"),
                                    ...
                                    for i, v in enumerate(rpa_08dc67_string):
                                        if i == 5:
                                            print(v)
                                        elif i == 2 or i == 8 or i == 9 or i == 11 or i == 13:
                                            continue
                                        elif i == 10:
                                            print(v)
                                        jp_list.append(v)
...
# -*- 保存Excel 文档
try:
    visual_block.Visual_Block_Info("保存Excel 文档", 13, 0, "正在执行")
    ...
    visual_block.Visual_Block_Info("保存Excel 文档", 13, 2, e)
...
# -*- 关闭Excel 文档
try:
    visual_block.Visual_Block_Info("关闭Excel 文档", 14, 0, "正在执行")
    ...
    visual_block.Visual_Block_Info("关闭Excel 文档", 14, 2, e)
...
# -*- 退出程序
try:
    visual_block.Visual_Block_Info("退出程序", 15, 0, "正在执行")

    exit(0)

except Exception as e:
    visual_block.Visual_Block_Info("退出程序", 15, 2, e)
